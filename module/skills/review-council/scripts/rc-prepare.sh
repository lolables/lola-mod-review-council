...
while [[ $# -gt 0 ]]; do
	case "$1" in
	--mode)
		[[ $# -lt 2 ]] && { json_output "skip" "--mode requires a value (code, specs, or auto)"; exit 0; }
		mode_override="$2"
		shift 2
		;;
	--scope)
		[[ $# -lt 2 ]] && { json_output "skip" "--scope requires a value (changed, all, range, paths, pr, or url)"; exit 0; }
		...
		;;
	--scope-value)
		[[ $# -lt 2 ]] && { json_output "skip" "--scope-value requires a value"; exit 0; }
		...
		;;
	--review-instructions)
		[[ $# -lt 2 ]] && { json_output "skip" "--review-instructions requires a value"; exit 0; }
		review_instructions="$2"
		shift 2
		;;
	--base)
		[[ $# -lt 2 ]] && { json_output "skip" "--base requires a value"; exit 0; }
		base_override="$2"
		shift 2
		;;
	--help)
