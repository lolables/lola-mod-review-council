...
<!-- This pack is for content production agents (Scribe,
     Herald, Envoy) only. Reviewer agents do not load
     this pack. -->

# Convention Pack: Content (Documentation, Blog, PR/Comms)

This is the content convention pack for the review-council
multi-persona code review system. It contains writing
standards for content-producing agents: The Scribe
(technical docs), The Herald (blog/announcements),
and The Envoy (PR/comms).

Each content agent focuses on the sections relevant
to its domain:
...
- **VB-001** [MUST] Content MUST use active voice as the
  default. "The analyzer flags unused imports" not "Unused
  imports are flagged by the analyzer." Passive voice is
  acceptable when the actor is genuinely unknown or
  unimportant.
...
- **VB-007** [SHOULD] Content SHOULD pass the "so what"
  ...
  not implied. "The linter flags unused imports" is a
  feature. "The linter tells you exactly which code paths
  are never exercised" is a benefit.
...
- **TD-001** [MUST] Technical documentation MUST be
  ...
  instructions ("Run `make install`") over passive
...
- **FA-003** [SHOULD] When researching upstream tool
  ...
  for accuracy, agents SHOULD use a knowledge layer MCP tool (if configured) as the
  primary discovery mechanism for cross-repo context.

- **FA-004** [MUST] After discovering relevant content
  via the knowledge layer, agents MUST read the
  target files directly to confirm exact numbers,
  flags, and capabilities before writing.
...
- **FT-002** [MUST] Use inline code spans (single
  backticks) for commands (`make install`), filenames
...
<!-- This section is intentionally empty in the canonical
     pack. Project-specific custom rules can be added in a
     separate file in your project's
     `.review-council/packs/` directory. Custom rules use
     the CR-NNN identifier prefix. -->
