...
token.go". Keep it under 120 characters — a longer one is rejected and costs
you a re-dispatch. Omit it and the first sentence of `description` is used
instead, which is why that sentence should stand on its own.
