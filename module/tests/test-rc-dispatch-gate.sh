...
if grep -qiE 'dispatch only .*(agents|identifiers)|only .*identifiers .*agents.? array' "$SKILL_MD"; then
	echo "  PASS: dispatch restricted to discovered array"; PASS=$((PASS+1))
else
	echo "  FAIL: no restriction of dispatch to the discovered array"; FAIL=$((FAIL+1))
fi

echo "Test: SKILL.md warns against un-suffixed legacy host agents"
if grep -qiE 'un-suffixed|legacy .*divisor' "$SKILL_MD"; then
	echo "  PASS: SKILL.md warns about legacy/un-suffixed agents"; PASS=$((PASS+1))
else
	echo "  FAIL: SKILL.md does not warn about legacy/un-suffixed agents"; FAIL=$((FAIL+1))
fi
...
if grep -qiE 'un-suffixed|legacy .*divisor|absent from that array|not in .*discovered' "$DELEGATE_MD"; then
	echo "  PASS: delegate.md forbids out-of-array dispatch"; PASS=$((PASS+1))
else
	echo "  FAIL: delegate.md does not forbid out-of-array dispatch"; FAIL=$((FAIL+1))
fi
