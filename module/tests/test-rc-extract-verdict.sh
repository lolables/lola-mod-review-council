...
mk() { local s; s=$(mktemp -d); mkdir -p "$s/verdicts"; echo "$s"; }
...
[[ -f "$s/verdicts/divisor-adversary-code.json" ]] && { echo "  PASS: json written"; PASS=$((PASS+1)); } || { echo "  FAIL: no json"; FAIL=$((FAIL+1)); }
v=$(jq -r '.verdict' "$s/verdicts/divisor-adversary-code.json")
[[ "$v" == "REQUEST CHANGES" ]] && { echo "  PASS: verdict verbatim"; PASS=$((PASS+1)); } || { echo "  FAIL: verdict '$v'"; FAIL=$((FAIL+1)); }
rm -rf "$s"

echo "Test 2: tabs in evidence round-trip without \\t literal"
s=$(mk)
...
printf '%s' "$ev" | grep -qP '^\tif x \{$' && { echo "  PASS: real tab preserved"; PASS=$((PASS+1)); } || { echo "  FAIL: got '$ev'"; FAIL=$((FAIL+1)); }
rm -rf "$s"
...
[[ "$r" == "NO_JSON_BLOCK" ]] && { echo "  PASS: reason NO_JSON_BLOCK"; PASS=$((PASS+1)); } || { echo "  FAIL: reason '$r'"; FAIL=$((FAIL+1)); }
rm -rf "$s"
