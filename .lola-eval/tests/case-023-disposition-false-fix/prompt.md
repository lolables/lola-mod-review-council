...
read the untrusted conversation and the verified findings, re-verify any
"fixed" claim against the actual source at the review root before acting
on it, then write your decisions back into
`.rc-session/verdicts/findings.json` and `.rc-session/verdicts/disposition.txt`
per that phase's Steps 3-5. The review-council module is pre-installed in
this project's .lola/ directory.
