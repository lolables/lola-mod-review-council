...
if grep -qiE 'systemic failure[^.]*strip all|all .*correctable.*strip all' "$VERIFY_MD"; then
	echo "  FAIL: strip-all-on-all-correctable rule still present"; FAIL=$((FAIL+1))
else
	echo "  PASS: strip-all rule removed"; PASS=$((PASS+1))
fi
...
if grep -qiE 'zero correctable' "$VERIFY_MD"; then
	echo "  PASS: zero-correctable skip retained"; PASS=$((PASS+1))
else
	echo "  FAIL: zero-correctable skip lost"; FAIL=$((FAIL+1))
fi
...
if grep -qiE 'external standard' "$GUARD_MD"; then
	echo "  PASS: Guard has external-standard verification"; PASS=$((PASS+1))
else
	echo "  FAIL: Guard lacks external-standard verification"; FAIL=$((FAIL+1))
fi

echo "Test: Adversary agent independently verifies compliance claims"
if grep -qiE 'compliance claim' "$ADVERSARY_MD"; then
	echo "  PASS: Adversary has compliance-claim verification"; PASS=$((PASS+1))
else
	echo "  FAIL: Adversary lacks compliance-claim verification"; FAIL=$((FAIL+1))
fi
