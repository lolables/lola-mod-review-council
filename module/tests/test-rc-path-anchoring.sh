...
if grep -qE 'REFERENCES_DIR=.*SKILL_DIR/references|references relative to .*SKILL_DIR|\$\{SKILL_DIR\}/references' "$SKILL_MD"; then
	echo "  PASS: references anchored on SKILL_DIR"; PASS=$((PASS+1))
else
	echo "  FAIL: references not anchored on SKILL_DIR"; FAIL=$((FAIL+1))
fi

echo "Test: MODULE_DIR-derived references path removed"
if grep -qE 'REFERENCES_DIR=.*MODULE_DIR/references' "$SKILL_MD"; then
	echo "  FAIL: MODULE_DIR-derived references path remains"; FAIL=$((FAIL+1))
else
	echo "  PASS: no MODULE_DIR references path"; PASS=$((PASS+1))
fi
...
if grep -qE 'AGENTS_DIR="\$\{MODULE_DIR\}/agents"' "$SKILL_MD"; then
	echo "  PASS: AGENTS_DIR derivation is concrete"; PASS=$((PASS+1))
else
	echo "  FAIL: AGENTS_DIR derivation missing or not concrete"; FAIL=$((FAIL+1))
fi

echo "Test: AGENTS_DIR is not a placeholder"
if grep -q 'AGENTS_DIR="<' "$SKILL_MD"; then
	echo "  FAIL: AGENTS_DIR placeholder found"; FAIL=$((FAIL+1))
else
	echo "  PASS: no AGENTS_DIR placeholder"; PASS=$((PASS+1))
fi
