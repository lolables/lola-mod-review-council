...
- The **one** exception: structured edits to
  `${session_dir}/verdicts/findings.json` — and only the `severity`,
  `status`, and `provenance.disposition` fields on existing finding objects,
  moving a finding between the `verified` and `stripped` arrays where the
  rules below call for it (Step 4). Never add, remove, or rewrite a
  finding's `description`, `recommendation`, or `evidence` fields — those
  belong to the reviewer and validator, not this phase.
...
>   `"DISPOSITION_SUPPRESSED_LOW"` — the same relocation `resolved` findings
>   get. This top-level `reason` is a different field from
...
`rc-prepare.sh` only implements this for GitHub today (see its
`# TODO(forge)` marker for GitLab). Before extending `pr-conversation.txt`
...
=== DISPOSITIONS ===
{per-finding: "Finding `{title}` from `{agent}` — {action: resolved | kept | suppressed-low} — {one-line reason, quoting the claim/hint}"}
