...
<!-- Base convention pack. Loaded ONLY when no language-specific
     pack (go.md, typescript.md, etc.) is available. Language
     packs include all base rules they need. -->

# Convention Pack: Base (Language-Agnostic)

This is the language-agnostic default convention pack for
the review-council multi-persona code review system. It contains universal
...
Language-specific packs are self-contained and include all
base conventions they need. When a language-specific pack is
active, load ONLY that pack. Load `base.md` only when no
language-specific pack is available.
